...
std::string extr_file_name(const std::string& pth)
   {
    ...
    return (i!=std::string::npos) ? pth.substr(i) : pth;
   }
...
void show_usage()
{
    ...
    std::cout << "   regtlb -q {0x714D710F,0x27B7,0x42BA,{0xA5,0x88,0x91,0xAD,0xC7,0xFF,0x4B,0x47}},1,0,0  (query tlb path by attributes)\n";
    std::cout << "             <GUID,MajorVer,MinorVer,lcid>\n";
    std::cout << "       -v: verbose\n";
    ...
    try{
        ...
        if( info )
        ...
        else if( query )
             {
              ...
              if(pth.empty()) return RET_NOTREG;
             }
